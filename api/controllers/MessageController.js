...
exports.groupMessages = (req, res) => {
    ...
    if (msgType) {
        ...
        if (invalidTypes.length > 0) {
            return res.status(400).json({ error: `msgType tidak valid: ${invalidTypes.join(", ")}. Gunakan salah satu dari: ${Object.keys(validTypes).join(", ")}` });
        }
    }
    ...
    fs.readFile(filePath, "utf8", (err, data) => {
        ...
        try {
            ...
            if (msgTypeList.length > 0) {
                const validTypeValues = msgTypeList.map(type => validTypes[type]); // Convert ke bentuk di file
